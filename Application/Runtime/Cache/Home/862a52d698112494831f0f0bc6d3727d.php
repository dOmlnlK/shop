<?php if (!defined('THINK_PATH')) exit();?><html xmlns="http://www.w3.org/1999/xhtml" xml:lang="en">
<head>
...
<div class="header w1210 bc mt15">
    ...
    <div class="logo w1210">
        ...
        <div class="search fl">
            <div class="search_form">
                ...
                <form action="" name="serarch" method="get" class="fl">
                    <input id="key" type="text" class="txt" value="<?php echo I('get.key', '请输入商品关键字'); ?>" />
        ...
        <div class="cart fl">
            <dl>
                <dt>
                    <a id="get_cart_list" href="<?php echo U('Cart/lst')?>">去购物车结算</a>
                </dt>
                <dd>
...
<script>
    viewPath = "<?php $ic=C('IMAGE_CONFIG');echo $ic['viewPath'] ?>"
    $("#get_cart_list").mouseover(
        function(){
            $.ajax({
                type:'get',
                dataType:'json',
                ...
                success:function (data) {
                    if(data){
                        ...
                        $(data).each(function (k,v) {
                            html += '<tr><td>' + "<img width='80px' src='" + viewPath + v.logo + "'></td>";
                            html += "<td>" + v.goods_name + "</td></tr>";
                        })
                        html += '</table>';
	...
	<div class="main w1210 mt10 bc">
		...
		<div class="breadcrumb">
			<h2>当前位置：<a href="">首页</a> >
				...
				<?php endforeach?>
				<?php echo ($goods_info["goods_name"]); ?></h2>
		...
		<div class="goods_left fl">
			...
			<div class="viewd leftbar mt10">
				...
				<div class="leftbar_wrap" id="view_history">
		...
		<div class="goods_content fl mt10 ml10">
			...
			<div class="summary">
				...
				<div class="preview fl">
					<div class="midpic">
						<a href="<?php echo ($viewPath); echo ($goods_info["mbig_logo"]); ?>" class="jqzoom" rel="gal1">   <!-- 第一幅图片的大图 class 和 rel属性不能更改 -->
                                       <?php showPic($goods_info['big_logo'])?><!-- 第一幅图片的中图 -->
						</a>
					</div>
					...
					<div class="smallpic">
						...
						<div class="smallpic_wrap">
							<ul>
								<li class="cur">
									<a class="zoomThumbActive" href="javascript:void(0);" rel="{gallery: 'gal1', smallimage: '<?php echo ($viewPath); echo ($goods_info["big_logo"]); ?>',largeimage: '<?php echo ($viewPath); echo ($goods_info["mbig_logo"]); ?>'}"><img src="<?php echo ($viewPath); echo ($goods_info["sm_logo"]); ?>"></a>
								</li>
								<li>
				...
				<div class="goodsinfo fl ml10">
					<ul>
						<li><span>商品编号： </span><?php echo ($goods_info["id"]); ?></li>
						...
						<li><span>会员价格：</span>
							<table border="1">
								<?php foreach($mp as $k=>$v):?>
								<tr><td><?php echo ($v["level_name"]); ?></td><td><?php echo ($v["price"]); ?></td></tr>
								<?php endforeach?>
							</table>
						</li>

						<li class="shop_price"><span>购买价格：</span> <strong style="font-size: 25px" id="true_price"></strong> <a href=""></a></li>

						<li><span>上架时间：</span><?php echo ($goods_info["addtime"]); ?></li>
							...
							<li class="product">
								<dl>
									...
									<dd>
										...
										<a <?php if($k1==0) echo 'class="selected"'?> href="javascript:;"><?php echo ($v1); ?> <input type="radio" name="goods_attr_id[<?php echo ($k); ?>]" value="<?php echo ($k1); ?>" <?php if($k1==0) echo 'checked="checked"'?> /></a>
										<?php endforeach?>
			...
			<div class="detail">
				...
				<div class="detail_bd">
					...
					<div class="comment detail_div mt10">
						<div class="comment_summary">
							...
							<div class="percent fl">
								<dl>
									...
									<dd><div class="hao_width"></div></dd>
								</dl>
								<dl>
									<dt>中评（<span class="zhong"></span>%）</dt>
									<dd><div class="zhong_width"></div></dd>
								</dl>
								<dl>
									<dt>差评（<span class="cha"></span>%）</dt>
									<dd><div class="cha_width" ></div></dd>
								</dl>
							</div>
							<div class="buyer fl">
								<dl id="impression">
						...
						<!-- 分页信息 start -->
						<div class="page mt20" id="page_list">
						...
						<div class="comment_form mt20">
							<form id="comment_form">
								<ul>
									...
									<li>
										<label for="">添加印象：</label>

										<input type="text" placeholder="添加多个印象请用,隔开" name="impression" size="30"/>
									</li>

									<li>
										<label for="">&nbsp;</label>
										<input type="hidden" name="goods_id" value="<?php echo ($goods_info["id"]); ?>">
										<input type="button" value="提交评论"  id='comment_btn' class="comment_btn"/>
									</li>
...
<div id="login_dialog" class="none" title="登录表单">
	<form id="login_form">
		<ul>
			...
			<li class="checkcode">
				<label for="">验证码：</label>
				<input type="text"  name="check_code" /><br />
				<img style="cursor:pointer;" onclick="this.src='<?php echo U('Member/checkCode'); ?>#'+Math.random();" src="<?php echo U('Member/checkCode'); ?>" />
			</li>
		</ul>
	</form>
</div>
...
<script>
	...
	function ajaxGetComment(page){
		$.ajax({
			type:"get",
			dataType:'json',
			url:"<?php echo U('Comment/ajaxGetComment?id='.$goods_info['id'],'',false)?>"+"/p/"+page,
			success:function (data) {
				...
				if(page == 1){
					...
					//添加印象
					if(data.impression){
						$(data.impression).each(function (k,v) {
							$("#impression").append("<dd><span>"+v.yx_name+"</span><em>("+v.yx_count+")</em></dd>");

							//添加已有印象多选框
							$("#impression_id").append("<input type='checkbox' name='impression_id[]' value='"+v.yx_name+"'>"+v.yx_name)
						})
				...
				$(data.data).each(function(k,v){
					...
					$(v.reply).each(function(k1,v1){
							hfhtml += '<li><img src="'+v1.avatar+'" width="50" />'+v1.username+' 【'+v1.addtime+'】 回复：<p>'+v1.content+'</p></li>'
					});

					html += '<div class="comment_items mt10"><div class="user_pic"><dl><dt><a href=""><img src="'+v.avatar+'" alt="" /></a></dt><dd><a href="">'+v.username+'</a></dd></dl></div><div class="item"><div class="title"><span>'+v.addtime+'</span><strong class="star star'+v.star+'"></strong></div><div class="comment_content">'+v.content+'</div><div class="btns"><a href="javascript:void(0);" onclick="do_reply(this, '+v.id+');" class="reply">回复('+v.reply_num+')</a><a href="" class="useful">有用('+v.click_count+')</a></div><div class="reply_form"></div><ul class="reply_container">'+hfhtml+'</ul></div><div class="cornor"></div></div>';
				...
				for(var i=1;i<=data.pageCount;i++){
					...
					pageNav += "<a onclick='ajaxGetComment("+i+")' href='javascript:void(0);' class="+cls+">"+i+"</a>"

				}
				$("#page_list").html(pageNav);
			}
...
<script>
	$.ajax({
		type:'GET',
		dataType:'json',
		url:"<?php echo U('getHistory?id='.$goods_info['id'])?>",
		success:function (data) {
			$("#view_history").html('');
			$(data).each(function (k,v) {

				var tag = "<dl><dt><a href=''>";
				tag += "<?php showPic('"+v.mid_logo+"')?>";
				tag += "</a></dt> <dd><a href=''>";
				tag +=  v.goods_name
				tag +=	"</a></dd></dl>"

				$("#view_history").append(tag);
			})
		}
	})
	...
	$.ajax({
		type:'GET',
		dataType:'json',
		url:"<?php echo U('ajaxGetTruePrice?id='.$goods_info['id'])?>",
		success:function (data) {
			$("#true_price").text('￥'+data+"元")
	...
	$("#comment_btn").click(function () {
		...
		$.ajax({
			...
			success:function (data) {
				if(data.status == 1){
					var html = '<div class="comment_items mt10 none"><div class="user_pic"><dl><dt><a href=""><img src="'+data.info.avatar+'" alt="" /></a></dt><dd><a href="">'+data.info.member+'</a></dd></dl></div><div class="item"><div class="title"><span>'+data.info.addtime+'</span><strong class="star star'+data.info.star+'"></strong></div><div class="comment_content">'+data.info.content+'</div><div class="btns"><a href="javascript:void(0);" onclick="do_reply(this,'+data.info.id+');" class="reply">回复(0)</a><a href="" class="useful">有用(0)</a></div><div class="reply_form"></div><ul class="reply_container"></ul></div><div class="cornor"></div></div>';
					// 把整个评论的字符串转化成jq的对象
					html = $(html);
					//重置评论框
	...
	function do_reply(btn,commentId) {
		var reply_form_div = $(btn).parent().next("div");
		var reply_form = '<br /><hr style="clear:both;margin-top:15px;" /><form><input type="hidden" name="comment_id" value="'+commentId+'" /><textarea name="content" style="width:100%;" rows="6"></textarea><br /><br /><input type="button" onclick="post_reply(this);" value="回复" /> <input onclick="close_reply(this);" type="button" value="取消" /></form>';
		// 放到 按钮后面
	...
	function post_reply(btn){
		...
		$.ajax({
			...
			success : function(data)
			{
				...
				{
					// 把回复的内容直接显示在页面中！！
					var html = '<li><img src="'+data.info.avatar+'" width="50" />'+data.info.member+' 【'+data.info.addtime+'】 回复：<p>'+data.info.content+'</p></li>';
...
<script>
    ...
    $.ajax({
        type:'GET',
        dataType:'json',
        url:"<?php echo U('Member/ajaxCkLogin')?>",
        success:function (data) {
            if(data.status==1){
                var li = "<li>您好," + data.member + "[<a href='"+"<?php echo U('member/logout')?>'>"+"退出</a>] " + "</li>";

            }else {
                var li = "<li>您好，欢迎来到京西！[<a href=<?php echo U('member/login')?>>登录</a>] [<a href=<?php echo U('member/regist')?>>免费注册</a>] </li>"

            }
            li += "<li class='line'>|</li>"+" <li><a href=<?php echo U('my/order')?>>我的订单</a></li>" + " <li class='line'>|</li>" + " <li>客户服务</li>";
            $("#login_info").append(li)
        }
