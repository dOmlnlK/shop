...
<div id="main-div">
    <div id="menu-list">
        <?php $model = D('privilege');$btns = $model->getMenu();?>

        <?php foreach($btns as $btn):?>
        <ul id="menu-ul">
            <li class="explode" key="02_cat_and_goods" name="menu">
            <?php echo ($btn["pri_name"]); ?>
                <ul>
                    <?php foreach($btn['children'] as $b):?>
                    <li class="menu-item"><a href="<?php echo U($b['controller_name'].'/'.$b['action_name'])?>" target="main-frame"><?php echo ($b["pri_name"]); ?></a></li>
                    <?php endforeach?>
                </ul>
            </li>
        </ul>
